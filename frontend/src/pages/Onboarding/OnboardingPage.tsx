...
import { FiArrowLeft, FiArrowRight, FiCheckCircle, FiTarget } from 'react-icons/fi';
...
export function OnboardingPage() {
  ...
  const submitLongWaitTimer = useRef<number | null>(null);
  ...
  }, [onboardingDraft, businessProfile, user]);
  ...
  const {
    ...
    // The resolver's generic inference can mismatch RHF's expected transformed values in this setup.
  ...
  useEffect(() => {
    return () => {
      if (submitLongWaitTimer.current) {
        window.clearTimeout(submitLongWaitTimer.current);
      }
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setOnboardingDraft(watchedValues as OnboardingFormValues);
    }, 300);
    return () => {
      window.clearTimeout(timer);
    };
  }, [setOnboardingDraft, user, watchedValues]);
  ...
  const goToStepTwo = async () => {
    const isStepOneValid = await trigger(['businessName', 'niche', 'timezone', 'language']);
  ...
  const onSubmit: SubmitHandler<OnboardingFormValues> = async (values) => {
    ...
    submitLongWaitTimer.current = window.setTimeout(() => {
      setSubmitStatus('This is taking longer than expected...');
    }, 45000);
    ...
    try {
      ...
      if (submitLongWaitTimer.current) {
        window.clearTimeout(submitLongWaitTimer.current);
        submitLongWaitTimer.current = null;
      }
      setSubmitStatus('');
  ...
  const stepTitle = step === 1 ? 'Business foundations' : 'Content preferences';
  ...
  return (
    <div className="onboarding-page">
      ...
      <form className="onboarding-form" onSubmit={handleSubmit(onSubmit)}>
        <div className="onboarding-progress glass-card">
        ...
        {submitStatus && <div className="onboarding-alert onboarding-alert--info">{submitStatus}</div>}
        {submitError && <div className="onboarding-alert onboarding-alert--error">{submitError}</div>}

        <div className="onboarding-actions">
          {step === 2 && (
            <Button type="button" variant="ghost" size="lg" leftIcon={<FiArrowLeft />} onClick={() => setStep(1)}>
              Back
            </Button>
          )}
          {step === 1 ? (
            <Button type="button" size="lg" rightIcon={<FiArrowRight />} onClick={goToStepTwo}>
              Continue to Step 2
            </Button>
          ) : (
